...
const AddSubject = ({ title, nameVal, attendedVal, totalVal, update, id ,setSubjects}: { title: string, nameVal: string, attendedVal: number, totalVal: number, update?: boolean, id: string ,setSubjects?:Function}) => {
  ...
  const updateSubject = ()=>{
    updateDoc(doc(db, "subjects", id), { name, attended: Number(attended),
      total: Number(total) }).then((res) => {
      setLoading(false);
    ...
    <>
      <div className="flex flex-col glass-div-front mt-4 max-sm:mt-0 mb-4 mr-4 ml-0 max-sm:h-full max-sm:w-full">
        ...
        <form onSubmit={(e) => handleSubmit(e)} method='post' className='flex flex-col p-4'>
          ...
          <input type="number" className='glass-input' value={total} onChange={(e) => setTotal(e.target.valueAsNumber)} name="total" id="total" placeholder='Enter the total course hours' required/>
          <label htmlFor="attended" className="mt-3">Attended Hours</label>
          <input type="number" name="attended" id="attended" value={attended} onChange={(e) => setAttended(e.target.valueAsNumber)} className='mb-3 glass-input' placeholder='Enter the attended hours' required/>
